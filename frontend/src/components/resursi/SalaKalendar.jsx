...
export default function SalaKalendar({ dostupnost, loading, onRefresh }) {
  ...
  if (!dostupnost || !dostupnost.sale?.length) {
    return <p className="empty-hint">Nema sala za prikaz kalendara na izabranoj lokaciji.</p>;
  }

  const { sale } = dostupnost;
  ...
  return (
    <div className="sala-kalendar">
      ...
      {sale.map((sala) => (
        <div key={sala.nazivSale} className="kalendar-sala-block">
