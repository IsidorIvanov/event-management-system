...
public class InventarPotrebeService {
    ...
    public InventarPotrebeValidacijaDto validirajPotrebeInventarom(Long dogadjajId) {
        ...
        for (DetektovanaPotrebaDto potreba : potrebe) {
            ...
            boolean imaDovoljno = josPotrebno == 0;

            if (imaDovoljno) {
                pokriveno++;
            } else if (dodeljeno > 0) {
                upozorenja.add(String.format(
                        "Delimično dodeljeno: %s (dodeljeno %d, potrebno %d, na stanju %d)",
                        potreba.getNazivResursa(), dodeljeno, potreba.getKolicina(), dostupno));
            } else {
                upozorenja.add(String.format(
